...
export default function ProjectPanel({ project, onClose }) {
  ...
  return (
    <div className="fixed top-0 right-0 w-full md:w-1/3 h-full bg-[#121224] text-[#e0e0e0] shadow-lg p-4 z-50 flex flex-col">
      <div className="flex justify-between items-start gap-4 mb-4">
        <h2 className="text-xl sm:text-2xl font-bold text-[#FFD600] break-words">
          {project.title}
        </h2>
        <button
          onClick={onClose}
          className="text-white hover:text-[#FFD600] text-2xl leading-none"
        >
          <IoClose />
        </button>
      </div>

      <div className="overflow-y-auto max-h-[85vh] pr-2 pb-16">
        {project.image && (
          <img
            src={project.image}
            alt={project.title}
        ...
        <div className="prose prose-invert prose-sm md:prose-base max-w-none mb-6 break-words">
